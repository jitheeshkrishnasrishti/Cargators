...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      /**
       * Track whether table has primary key as tables without one can create problems
       */
      ...
      var entityName = tableName.replace(/^(tbl\.)/,'') + 'Entity';
      if(!self.options.typescript){
        ...
        /**
         * initialise param entity within the entity class code being created.
         */
        text[table] += spaces + "let " + entityName + " = sequelize.define('" + tableName + "', {\n";
      } else {
      ...
      if(false === hasPrimaryKey){
        /**
         * If there is no primary key then remove the default id that has been manufactured
         *
         * @type {string}
         */
        text[table] += spaces + "//@see: https://github.com/sequelize/sequelize/issues/1026#issuecomment-54877327\n";
        text[table] += spaces +  entityName + ".removeAttribute('id');\n";
      }
      /**
       * now return the concatenated entity within the entity class code being created
       * @type {string}
       */
      text[table] += spaces + "return " + entityName + ";\n";
      text[table] += "};\n";
      _callback(null);
    }, function(){
